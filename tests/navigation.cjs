...
(async () => {
  ...
  try {
    ...
    await page.locator('[data-os="prev-period"]').click();
    const calendar = await page.locator('.os-week-plan').innerText();
    await page.locator('.os-week-row').nth(3).click();
    await page.goBack();
    await page.locator('.os-week-plan').waitFor();
    assert.equal(await page.locator('.os-week-plan').innerText(), calendar);
    ...
    console.log('PASS: phone Back, week context, text guides, timer, logs, forward, reload, routine details');
